...
class ProjectedCoordinate {
	public:
		...
		explicit ProjectedCoordinate( const GPSCoordinate& gps ) {
			x = ( gps.longitude + 180.0 ) / 360.0; // * ( 1 << z );
			y = ( 1.0 - log( tan( gps.latitude * M_PI / 180.0 ) + 1.0 / cos( gps.latitude * M_PI / 180.0 ) ) / M_PI ) / 2.0; // * ( 1 << z );
		}
		
		GPSCoordinate ToGPSCoordinate() const {
			GPSCoordinate gps;
			gps.longitude = x /* / ( 1 << z ) */ * 360.0 - 180;
			const double n = M_PI - 2.0 * M_PI * y; // / ( 1 << z );
