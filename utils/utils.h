...
static bool pointInPolygon( int numPoints, const DoublePoint* points, DoublePoint testPoint ) {
	bool inside = false;
...
static bool clipHelper( double directedProjection, double directedDistance, double* tMinimum, double* tMaximum ) {
...
static bool clipEdge( ProjectedCoordinate* start, ProjectedCoordinate* end, ProjectedCoordinate min, ProjectedCoordinate max ) {
...
static void write_unaligned_unsigned( char** buffer, unsigned data, int bits, int* offset ) {
	if ( bits != 32 )
		assert( ( data & ( ~( ( 1u << bits ) - 1 ) ) ) == 0 );
